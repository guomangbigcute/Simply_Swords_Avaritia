...
public class SimpleSwordAvaritiaConfig {
    ...
    public static void loadConfig() {
        //System.out.println("Loading common Simple Sword Avaritia config");
        JsonObject json;
        json = Config.getJsonObject(Config.readFile(new File("config/simple_sword_avaritia/loot_config.json5")));
    ...
    public static void generateConfigs(boolean overwrite) {
        //System.out.println("Generating common Simple Sword Avaritia config");
        ...
        for (String key : LOOT_OPTIONS.keySet()) {
            if (i == 0) {
                ...
                config.append("// If 'add_weapons_to_loot_tables' is enabled in the simple_sword_avaritia_main config, this mod will attempt to inject its loot into    \n");
        ...
        config.append("}");
        Config.createFile("config/simple_sword_avaritia/loot_config.json5", config.toString(), overwrite);
    }
}