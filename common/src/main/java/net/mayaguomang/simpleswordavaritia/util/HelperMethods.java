...
import net.mayaguomang.simpleswordavaritia.effect.instance.SimpleSwordAvaritiaStatusEffectInstance;
...
import static net.mayaguomang.simpleswordavaritia.SimpleSwordAvaritia.minimumSpellPowerVersion;
...
public class HelperMethods {
    ...
    public static Style getStyle(String styleType) {
        ...
        int rgbInfinity =           0xFC5454;  // Red color for Infinity weapons (matches Avaritia)
        ...
        Style INFINITY =            Style.EMPTY.withColor(TextColor.fromRgb(rgbInfinity));
    ...
    //Check if item is a unique 2H weapon - DISABLED: All UNIQUE items are not registered
    public static boolean isUniqueTwohanded(ItemStack stack) {
        return false;
        /*
// DISABLED:         return stack.isOf(ItemsRegistry.SOULPYRE.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.SOULKEEPER.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.TWISTED_BLADE.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.HEARTHFLAME.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.SOULRENDER.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.SLUMBERING_LICHBLADE.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.WAKING_LICHBLADE.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.AWAKENED_LICHBLADE.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.BRIMSTONE_CLAYMORE.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.ICEWHISPER.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.ARCANETHYST.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.THUNDERBRAND.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.WHISPERWIND.get()) ||
// DISABLED:                 stack.isOf(ItemsRegistry.WATCHER_CLAYMORE.get());
        */
    }
    ...
    public static SimpleSwordAvaritiaStatusEffectInstance incrementSSAStatusEffect(
            LivingEntity livingEntity,
            StatusEffect statusEffect,
            int duration,
            int amplifier,
        ...
        SimpleSwordAvaritiaStatusEffectInstance statusReturn;
        if (livingEntity.hasStatusEffect(statusEffect)) {
            int currentDuration = livingEntity.getStatusEffect(statusEffect).getDuration();
            int currentAmplifier = livingEntity.getStatusEffect(statusEffect).getAmplifier();

            if (currentAmplifier >= amplifierMax) {
                statusReturn = new SimpleSwordAvaritiaStatusEffectInstance(statusEffect, Math.max(currentDuration, duration),
        ...
        statusReturn = new SimpleSwordAvaritiaStatusEffectInstance(statusEffect, duration, 0,
                false, false, true);
        livingEntity.addStatusEffect(statusReturn);
        return statusReturn;
    }
    ...
    public static float commonSpellAttributeScaling(float damageModifier, Entity entity, String magicSchool) {
        if (Platform.isModLoaded("spell_power") && Platform.isFabric())
            if ((entity instanceof PlayerEntity player) && Config.getBoolean("compatEnableSpellPowerScaling", "General",ConfigDefaultValues.compatEnableSpellPowerScaling) && SimpleSwordAvaritia.passVersionCheck("spell_power", minimumSpellPowerVersion))
                return SimpleSwordAvaritiaExpectPlatform.getSpellPowerDamage(damageModifier, player, magicSchool);
