...
public class WideOrbitingEffect extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity livingEntity, int amplifier) {
        if (!livingEntity.getWorld().isClient) {
            ...
            double radius = baseRadius + (amplifier * 0.5); // Increase the radius based on the amplifier
            // Increase the number of particles based on the amplifier to fill in the gaps
            int particleCount = 5 + (amplifier * 3); // Example: Increase particle count with the amplifier
